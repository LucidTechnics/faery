...
rule.type = function()
{
	...
	{
		type = "java.lang.String";
		this.persistable.addParameterValue("semanticType", "Packages.airlift.meta.AttributeMetadata.Semantic.VERYLONGTEXT");
	...
	else if ('text'.equalsIgnoreCase(this.value.toLowerCase()) === true)
	{
		type = "java.lang.String";
	}
	else 
	{
		type = join(this.value, convertToParameterName).toLowerCase();
	}

	if (this.value &&
		(
		this.value.equalsIgnoreCase("collection") === true ||
		this.value.equalsIgnoreCase("list") === true ||
		this.value.equalsIgnoreCase("tuple") === true
		)
	   )
	{
		type = "java.util.List";
	...
	if (this.value &&
		  (
		   this.value.equalsIgnoreCase("set") === true ||
		   this.value.equalsIgnoreCase("group") === true
		  )
	   )
	{
		type = "java.util.Set";
...
function writeBrowserResourceMeta(_metaName, _contents)
{
	var fileSeparator = Packages.java.io.File.separator;
	
	var dir = project.getProperty("war") + fileSeparator +
...
function writeBrowserAttributeMeta(_metaName, _contents)
{
	var fileSeparator = Packages.java.io.File.separator;

	var dir = project.getProperty("war") + fileSeparator +
		  "gen" + fileSeparator + "meta" + fileSeparator + "a";
	var file = new Packages.java.io.File(dir + fileSeparator + _metaName.toLowerCase() + ".js");
	file["delete"]();
	writeFile(file, _contents);
}
...
while (resourceNameToResourceModelMapEntrySetIterator.hasNext() === true)
{
	...
	if (resourceModel.isAbstractResource() === false)
	{
		...
		writeBrowserResourceMeta(resourceModel.getClassName(), resourceMetaDataString);
		...
		writeBrowserAttributeMeta(resourceModel.getClassName(), attributeMetadataString);
	}
	else
	{
